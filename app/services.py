...
async def add_payment(tg_id: int, provider: str, amount_uzs: int, status: str, plan_days: int, ext_id: str | None = None):
    async with Session() as s:
        s.add(Payment(tg_id=tg_id, provider=provider, amount=amount_uzs, status=status, plan_days=plan_days, ext_id=ext_id))
        await s.commit()
...
async def get_or_create_txn(provider: str, ext_id: str, tg_id: int, plan_days: int, amount_uzs: int):
    async with Session() as s:
        ...
        if txn:
            return txn
        txn = Txn(provider=provider, ext_id=ext_id, tg_id=tg_id, plan_days=plan_days, amount_uzs=amount_uzs, state="created")
        s.add(txn)
        await s.commit()
        return txn
